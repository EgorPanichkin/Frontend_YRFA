...
export const ModalPersonal = (props) => {
  const { title, confirmation, setConfirmationExit, setConfirmationId, handleConfirmDelete } = props
  ...
  return (
    <div className={style.exitNotification}>
      ...
      <div className={style.exitNotificationButtons}>
        ...
        {confirmation === 'exit' ? (
          <CustomButton color='default' onClick={() => setConfirmationExit(false)}>
            <Typography>Отменить</Typography>
          </CustomButton>
        ) : (
          <CustomButton color='default' onClick={() => setConfirmationId(false)}>
            Отменить
          </CustomButton>
        )}
      </div>
