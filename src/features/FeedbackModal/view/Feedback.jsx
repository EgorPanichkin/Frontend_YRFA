...
export const Feedback = () => {
  ...
  const { handleInputChange, isButtonDisabled, inputValues, counter } = FeedbackValidation()
      ...
      {feedbackModal && (
        <ModalWrapper onCloseModal={() => setFeedbackModal(false)}>
          <div className={style.questio}>
            <Cross className={style.closeModal} onClick={() => setFeedbackModal(false)} />
            <form onSubmit={handleSubmit} className={style.questioForm}>
              <Typography variant='h2' weight='600' className={style.questioTitle}>
              ...
              <CustomButton type='submit' disabled={isButtonDisabled} className={style.questioBtn}>
